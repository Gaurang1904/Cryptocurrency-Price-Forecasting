...
def print_future_predictions(model, train_df, days=7):
    ...
    try:
        # Use the last available sequence for context
        context_df = train_df.tail(24).copy()
        
        # Make future predictions using the proper method
        future_pred = model.predict(df=context_df)
        ...
        # Get the number of predictions available (might be less than requested days)
        available_preds = min(days, len(future_pred))
        
        for i in range(available_preds):
        ...
        print("This might be due to the model configuration or data format.")
        print("Future predictions are optional and don't affect the main model evaluation.")
        print("="*50)
        return None
